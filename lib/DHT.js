...
const DEFAULT_OPTIONS = {
	...
	sendStoreRequest: (peer, key, value, id) => {throw new Error(`sendStoreRequest abstract`)},
	// Store a value
	storeValue: (key, value, id) => {throw new Error(`storeValue abstract`)},
	// Send a retrieve request to a peer
	sendGetRequest: (peer, key, id) => {throw new Error(`sendGetRequest abstract`)},
	// Retrieve a value
	getValue: (key, id) => {throw new Error(`getValue abstract`)},
};
...
class DHT extends EventEmitter {
	...
	handleStoreRequest(peer, key, value, id, resp_fn){
		if(this.options.autoUpdateKBuckets)
			this.touchPeer(peer);
		...
		if(dist <= farthest_dist){
			// Store the value
			this.options.storeValue(key, value, id);
		}else{
			// Other peers are closer, don't store
		}
		resp_fn(dist <= farthest_dist); // TODO: Response?
	}
	/**
	...
	handleGetRequest(peer, key, id, resp_fn){
		if(this.options.autoUpdateKBuckets)
			this.touchPeer(peer);
		const v = this.options.getValue(key, id);
		resp_fn(v);
	}
}
