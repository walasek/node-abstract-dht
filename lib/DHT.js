...
class DHT extends EventEmitter {
	...
	async nodeLookup(id, n=1, break_on_find=true){
		return new Promise(async (res, rej) => {
			...
			// Initially assume all peers are potential candidates
			peers.forEach(p => _checkCandidate(p));
	...
	async putValue(key, value){
		...
		// 2a. If found less than k then store locally
		// Automatic re-broadcast is not performed!
		const id = this.hash(key);
		const closest = await this.nodeLookup(id, this.options.k, false, false);
		if(closest.length < this.options.k && closest.findIndex(p => Buffer.compare(this.options.id, p.id) == 0) == -1)
			await this.options.storeValue(key, value);
		const sent_to = [];
	...
	async handleStoreRequest(peer, key, value, id){
		...
		if(dist <= farthest_dist || closest.length == 0){
			// Store the value
	...
	async getValue(key){
		const id = this.hash(key);
		const local = await this.options.getValue(key);
		if(local)
			return local;
